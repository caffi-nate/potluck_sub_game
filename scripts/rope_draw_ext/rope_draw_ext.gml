/// @description rope_draw(id, texture, color, alpha)
/// @param id
/// @param  texture
/// @param  color
/// @param  alpha
function rope_draw_ext(argument0, argument1, argument2, argument3) {
	...
	for (i=0; i<_cuts-1; i++) {
	  ...
	  for (j=a; j<b; j++) {
	    ...
	    if (j = b-1) {
		  ...
		  // test: get the center of the rope
		  if (j == (a + floor(0.5*b)))//(_cuts-2)*0.5){
		  {
			  //show_debug_message("halfway");
			  //rope_middle_x = 0;
			  //rope_middle_y = 0;
			  rope_middle_x = rp_x[_id,j];
			  rope_middle_y = rp_y[_id,j];
		  }
	  }
	  ...
	  for (j=a; j<b+1; j++) {
	    ...
	    draw_vertex_texture_colour(_x1 - VX, _y1 - VY, j, 0, _color, _alpha);
	    draw_vertex_texture_colour(_x2 - VX, _y2 - VY, j, 1, _color, _alpha);
	  }
