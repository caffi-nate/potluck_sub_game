function player_underwater(argument0, argument1) {
	movementSpeed = 3//2.5//3;
	acceleration = 0.1//0.05; //0.2
	var TURNINGSPEED = 150; // instantaneous turning test //8//4;

	var XDIR = argument0;
	var YDIR = argument1;

	var OVERENCUMBERED_PENALTY = 1* (heldItem != noone);

	if ((abs(XDIR) + abs(YDIR)) > 0){
		newAngle = point_direction(x,y,x + XDIR,y + YDIR);
	}

	var CURRENT_SPEED = abs(hSpeed) + abs(vSpeed);
	...
	if (newAngle != -4){
		...
		if (ANGLEDIFFERENCE < 135 || ANGLEDIFFERENCE > 225){
			...
			//hSpeed = lengthdir_x(movementSpeed, facingDirection);
			//vSpeed = lengthdir_y(movementSpeed, facingDirection);
		
			var ADJUSTED_SPEED = movementSpeed - OVERENCUMBERED_PENALTY;
		
			hSpeed = approach(hSpeed,lengthdir_x(ADJUSTED_SPEED,facingDirection),acceleration);
			vSpeed = approach(vSpeed,lengthdir_y(ADJUSTED_SPEED,facingDirection),acceleration);	
		
			// when travelling upwards, get penalised more for carrying that weight
			//vSpeed = max(vSpeed, vSpeed + OVERENCUMBERED_PENALTY);
		}
